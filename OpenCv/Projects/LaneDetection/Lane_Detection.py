...
def getLaneCurve(img, display=2, warp=True):
    ...
    ####### 3- Getting the Curve #######################################################################################
    if warp:
        curvePoint, imgHist = utils.getHistogram(warp_img, 0.9, 1, True)
        centerPoint, imgHist2 = utils.getHistogram(warp_img, 0.5, 0.25, True)
    else:
        curvePoint, imgHist = utils.getHistogram(mask_img, 0.6, 1, True)
    ...
    imgLaneColor = cv2.bitwise_and(imgLaneColor, imgLaneColor, mask=imgMask)
    ...
    if display == 1:
        ...
        imgStacked = stackImages(0.5, ([img, imgWarpPoints, mask_img, warp_img],
                                             [imgHist, imgLaneColor, imgResult, imgBlack]))
